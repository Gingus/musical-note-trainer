...
# Updated/Adapted to use for Harmonica tab by Phil Minard

import sys
import random
import math
# import os
# installed pipwin to be able to install PyAudio
import PyAudio
...
from random import *
import numpy
# from scipy.signal blackmanharris, fftconvolve
from scipy.signal.windows import blackmanharris, fftconvolve
# https://docs.scipy.org/doc/scipy/search.html?q=blackmanharris # Not sure if this is being used!
from numpy import argmax, sqrt, mean, diff, log
from matplotlib.mlab import find  # This is needed for 'freq_from_autocorr' to work so check the documentation



class SoundRecorder:
    """Opens an audio stream and converts it to human readable format from frequency
...
def loudness(chunk):
    """Dealing with the volume element"""
...
def build_default_tuner_range():
    """Assigning music note values to frequency range Dictionary"""
    return {65.41: 'C2',
            69.30: 'C2#',
            73.42: 'D2',
            77.78: 'E2b',
        ...
        while trys != 0:
            ...
            for n in range(0, screen_size[0], stepsize+1):
                ...
                try: 
                    ...
                    inputnote == 0  # This currently has no effect!
                ...
                # find the closest note in the keyed array
                position = ((n), (screen_size[1]-(int(screen_size[1]/(frequencies[top_note]-frequencies[bot_note]) *
                ...
                if stepchange:  # go to start of the loop if the step size is altered
                ...
                if needle:
                    pygame.draw.line(screen, stepsizecolor, centrescreen, (centrescreen[0]+((frequencies[targetnote]
                    - inputnote)*20), centrescreen[1]-200), 3)
                ...
                err = abs(frequencies[targetnote]-inputnote)
                text2 = font2.render("Range: " + str(tunerNotes[frequencies[bot_note]]) + " - " +
                                     str(tunerNotes[frequencies[top_note]])
                   + "    Show Notes(n):" + str(shownotes) + "    Circles(c):" + str(circ) 
                   + "    Lines(l):" + str(line) + "    Stepsize(v<>b):" + str(stepsize)
                   + "    Fill Circles(f):" + str(not fill) + "    Rand Clr(r):" + str(col) + "    Clr Screen(m):"
                                     + str(cls) + "   Lev: " + meter[1:int(20-signal_level)], bool(1), (255, 255, 0))
                ...
                if shownotes:            
                    ...
                    err = abs(frequencies[targetnote]-inputnote)
                    if err < 1.5:
                        text = font.render(str(tunerNotes[frequencies[targetnote]]), bool(1), (0, 255, 0))

                    if 1.5 <= err <= 2.5:
                        text = font.render(str(tunerNotes[frequencies[targetnote]]), bool(1), (255, 255, 255))

                    if err > 2.5:
                        text = font.render(str(tunerNotes[frequencies[targetnote]]), bool(1), (255, 0, 0))
